...
do -- Load items
    ...
    local keybind = Enum.KeyCode.Insert -- Default keybind
    local isDragging = false
    local expanded = false
    local currentTab = "Keybind"
    local windows = {}
    ...
    local function toggleWindow(window)
        ...
        window.Content.Visible = expanded
        window.Main.Expand.Rotation = expanded and 270 or 90
        tweenSize(window.Main, UDim2.new(0, 300, 0, expanded and 222 or 22), 0.2)
    end

    local function setTab(tabName)
        currentTab = tabName
        for _, tab in pairs(Tabs.Items:GetChildren()) do
            if tab:IsA("TextButton") then
                tab.BackgroundTransparency = tab.Text == tabName and 0.8 or 1
            end
        end
    ...
    keybindText.Size = UDim2.new(0, 200, 0, 20)
    keybindText.Font = Enum.Font.Code
    ...
    Main.Expand.MouseButton1Click:Connect(function()
        toggleWindow(Presets)
    end)
